...
for a=1:size(XALL,1)
    for j=1:33
        RR_PDif(a,j)=(abs(RRyear_sim(a,j)-RRyear(1,j)))/RRyear(1,j)*100;
        ArnoldBFI_PDif(a,j)=(abs(ArnoldBFI_sim(a,j)-ArnoldBFI(1,j)))/ArnoldBFI(1,j)*100;
        SFDC_PDif(a,j)=(abs(SFDC_sim(a,j)-SFDC(1,j)))/SFDC(1,j)*100;
        CoM_PDif(a,j)=(abs(CoM_sim(a,j)-CoM(1,j)))/CoM(1,j)*100;
    end
end
...
save bear_hymod_5000hyd_hydr_sig_HydSigRun3.mat RR_PDif ArnoldBFI_PDif SFDC_PDif CoM_PDif
save bear_hymod_5000hyd_q_EP_snow_HydSigRun3.mat EP_obs_yr qoyr_down EP_sim_yr qsyr_down
save bear_hymod_5000hyd_ofs_best_snow_HydSigRun3.mat SFDCE_high_yr SFDCE_mid_yr SFDCE_low_yr SFDCEP_high_yr SFDCEP_mid_yr SFDCEP_low_yr %SFDCE_high_whole SFDCE_mid_whole SFDCE_low_whole %EP_obs EP_obs_yr EP_sim_yr EP_sim
save bear_hymod_5000hyd_ofs_loop_snow_HydSigRun3.mat SFDCE SFDCEP %SFDCE.high_yr_loop SFDCE.mid_yr_loop SFDCE.low_yr_loop SFDCEP.high_yr_loop SFDCEP.mid_yr_loop SFDCEP.low_yr_loop
save bear_hymod_5000hyd_objFun_snow_HydSigRun3.mat NSEyear KGEyear WByear NSE_whole KGE_whole RSMEyear %RREyear
save bear_SA_5000sim_val_snow_HydSigRun3.mat NSEtop_val NSEtop_index KGEtop_val KGEtop_index Best_KGE_run Best_NSE_run
save bear_SA_5000sim_snow_HydSigRun3.mat PE_value %data length by 1
save bear_SA_5000pars_snow_HydSigRun3.mat XALL %N by par  nr
...
save bear_sens_yr_run5000_hydr_sig_HydSigRun3.mat RRyearSi RRyearSTi ArBFIyearSi ArBFIyearSTi SFDCyearSi SFDCyearSTi CoMyearSi CoMyearSTi 
save bear_sens_yr_run5000_snow_HydSigRun3.mat WByearSi WByearSTi NSEyearSi NSEyearSTi KGEyearSi KGEyearSTi RMSEyearSi RMSEyearSTi %RREyearSi RREyearSTi %MyearSi MyearSTi %year nr by par nr
save bear_sens_yr_hyd_SFDCE_run5000_snow_HydSigRun3.mat SFDCE_high_yearSi SFDCE_high_yearSTi SFDCE_mid_yearSi SFDCE_mid_yearSTi SFDCE_low_yearSi SFDCE_low_yearSTi
...
save bear_sens_QSTi_KGEwhole_run5000_snow_HydSigRun3.mat QSi QSTi KGEwholeSi KGEwholeSTi
