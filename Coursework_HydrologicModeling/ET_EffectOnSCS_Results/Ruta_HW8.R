...
#plot(Wat_Y,Yearly_aver_obs,type="b",xlab=paste("Years"),ylab=paste("Q (cfs)"),ylim=c(min(Yearly_aver_sim),2300),main="Observed vs predicted annual \ndaily average streamflow (1998-2017)\n(with Hammon ET)",lwd=1.5,lty=1)#,pch=16,cex=0.75)
#lines(Wat_Y,Yearly_aver_sim,type="b",lwd=1.5,col="blue",lty=1)
#legend(x="topleft",legend=c("Observed","Predicted"),col=c("black","blue"),lty=c(1,1),cex=0.6,bty = "n")
#legend(x="topright",legend=paste("NSE=",round(NSE_ann_new,3)," Bias= ",round(Bias_ann_new,3)),cex=0.6,bty = "n")

#Mon=c("Oct","Nov","Dec","Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep")
#plot(1:12,Mon_obs,type="b",xaxt = "n",xlab=paste("Month"),ylab=paste("Q (cfs)"),ylim=c(min(Mon_sim),max(Mon_obs)),main="Observed vs predicted montly \ndaily average streamflow (1998-2017)\n(with Hammon ET)",lwd=1.5,lty=1)#pch=16,cex=0.75)
#lines(1:12,Mon_sim,type="b",lwd=1.5,col="blue",lty=1)
#axis(1, at=1:12, labels=Mon)
#legend(x="topleft",legend=c("Observed","Predicted"),col=c("black","blue"),lty=c(1,1),cex=0.6,bty = "n")
#legend(x="topright",legend=paste("NSE=",round(NSE_mon_new,3)," Bias= ",round(Bias_mon_new,3)),cex=0.6,bty = "n")
...
write.csv(out_val, file = "Basijokaite_HW8.csv",append=T)
...
for (i in seq(1,len-1)){
  #mo_nr=date$mon[i]+1
  #ET_crop2[i]=ET_val_site_cm[i]*KU[mo_nr]
  #if(ET_crop2[i]>UNSAT2[i]){
  #  ET_cor2[i]=UNSAT2[i]
  #}else{
  #  ET_cor2[i]=ET_crop2[i]
  #}
  #UNSAT2[i+1]=UNSAT2[i]+I[i]-ET_cor[i]
...
#legend(x="topright",legend=paste("NSE=",round(NSE_ann_new2,3)," Bias= ",round(Bias_ann_new2,3)),cex=0.6,bty = "n")
...
axis(1, at=1:12, labels=Mon)
legend(x="topleft",legend=c("Observed","Hammon ET","Oudin ET"),col=c("black","blue","red"),lty=c(1,1,1),cex=0.6,bty = "n")
#legend(x="topright",legend=paste("NSE=",round(NSE_mon_new2,3)," Bias= ",round(Bias_mon_new2,3)),cex=0.6,bty = "n")
...
#plot(1:365,Discharge_obs[1:365],type="l",xlab=paste("Days in water year"),ylab=paste("Q (cfs)"),ylim=c(min(Q_sim_cfs2),max(Q_sim_cfs)),main="Observed vs predicted streamflow hydrograph (1998)",pch=16,cex=0.75)
#points(1:365,Q_sim_cfs[1:365],type="l",col = "green",cex=0.75)
#points(1:365,Q_sim_cfs2[1:365],type="l",col = "red",cex=0.75)
