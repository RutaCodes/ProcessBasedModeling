...
#Monthly_yr_aver - daily month averages throughout all years
...
Monthly_loop <- function(date,Precip){
  ...
  for (i in seq(Start_year,Last_year-1)){
    ...
    for (j in seq(1,length(Months))){
      ...
      Monthly_P=Precip[x]
      Monthly_P_aver[count2,count]=mean(Monthly_P) #monthly daily average
    }
    Monthly_P=0.0
    count2=0.0
    Monthly_yr_aver=mean(Monthly_P_aver)
  }
  for (i in seq(1,12)){
